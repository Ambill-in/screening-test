 ...
 *   - apply receipt-type rules (TDS, etc.) to the resulting full record
...
export function processPayment(context: PipelineContext): PaymentRecord {
  ...
  if (method === 'patch') {
    ...
    // Receipt rules run on the merged record so a TDS payment keeps
    // amount_paid at zero even when the patch omits receipt_type.

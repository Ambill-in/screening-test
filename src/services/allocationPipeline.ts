...
export function processPayment(context: PipelineContext): PaymentRecord {
  ...
  const stripped = stripManagedFields(data, MANAGED_FIELDS) as Partial<PaymentRecord>;

  if (method === 'patch') {
    if (!existing) {
      throw new Error('Payment not found');
    }
    const merged = mergePatch(existing, stripped);
    const normalized = normalizePayload(merged as unknown as Record<string, unknown>) as unknown as PaymentRecord;
    return applyReceiptTypeRules(normalized);
  }

  enforceOrgScope(user, query);
  const normalized = normalizePayload(stripped as unknown as Record<string, unknown>) as unknown as Partial<PaymentRecord>;
  return applyReceiptTypeRules(normalized) as PaymentRecord;
}

...
import { toMoney, moneyEquals } from '../lib/money';
...
export function validateAllocationTotal(
  ...
  if (!moneyEquals(toMoney(total), toMoney(paymentAmount))) {
...
export function processPayment(context: PipelineContext): PaymentRecord {
  ...
  if (method === 'patch') {
    ...
    const merged = mergePatch(
      existing,
      normalizePayload(data as Record<string, unknown>) as Partial<PaymentRecord>
    );
    const stripped = stripManagedFields(merged, MANAGED_FIELDS) as PaymentRecord;
    return applyReceiptTypeRules(stripped);
  }

  enforceOrgScope(user, query);

  const normalized = normalizePayload(data as Record<string, unknown>) as Partial<PaymentRecord>;
  const stripped = stripManagedFields(normalized, MANAGED_FIELDS) as Partial<PaymentRecord>;
  return applyReceiptTypeRules(stripped as PaymentRecord);
}
